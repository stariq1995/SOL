...
class RocketFuelTopology(Topology):
    """ Class for a rocket fuel topology"""

    def __init__(self, name, graph=None, numFlows=0):
        super(RocketFuelTopology, self).__init__(name, graph, numFlows)

    def computeGravityODTM(self, nodePairs):
        """ Computes the gravity model traffic matrix for this topology

        :param nodePairs: compute gravity TM for these IE pairs
        """
        tm = dict()
        G = self._graph  # ease of typing
        tot_population = sum([G.node[x]['population'] for x, _ in nodePairs])

        for i, e in nodePairs:
            ti_in = G.node[i]['population']
            # tj_out = 0
            # Try to access the population of the sink
            try:
                tj_out = G.node[e]['population']
            except KeyError:
                # See if we can access the population of the sinkParent
                tj_out = G.node[G.node[e]['sinkParent']]['population']
            tm[(i, e)] = ((float(ti_in * tj_out) / (tot_population ** 2))
                          * self.getNumFlows())
        return TrafficMatrix(tm)

    def generateTrafficMatrix(self, nodePairs, model='normal'):
        """ Generates a traffic matrix

        :param nodePairs: the node pairs for which the traffic should be
            flowing
        :param model: the way to generate the traffic matrix. Default is the
            gravity model
        :raises ValueError: if unknown model is specified
        """
        if model == 'gravity':
            return self.computeGravityODTM(nodePairs)
        elif model == 'uniform':
            return self.computeUniformODTM(nodePairs)
        elif model == 'dirichlet':
            return self.computeDirichletODTM(nodePairs)
        elif model == 'normal':
            return self.computeUnifromPlusNormalODTM(nodePairs)
        else:
            raise ValueError('Unknown traffic matrix model')

    def generateIEPairs(self, minShortestPath=1, minDegree=1, hasSinks=True):
        """  Default method to generate ingress-egress pairs

        :param minShortestPath: require at least this many hops between
            ingress and egress to be considered a valid IE pair.
            Useful for avoiding IE pairs with no possible paths
        :param minDegree: force the source nodes to have a degree larger
            than specified value
        :param hasSinks: set to true if the topology has been modified to
            include superSinks and they should be used as egress nodes
        :return: list of ingress-egress pairs
        :rtype: list of tuples
        """
        result = []
        G = self._graph
        assert nx.is_connected(G.to_undirected())
        sources = [x for x in G.nodes_iter() if self.isSwitch(x)]
        sinks = list(sources)
        if hasSinks:
            for n in sources:
                for s in G.successors(n):
                    if 'superSink' in G.node[s]:
                        sinks.remove(n)
                        sinks.append(s)
        degrees = G.to_undirected().degree()
        sl = nx.all_pairs_shortest_path_length(G)
        for s in sources:
            for t in sinks:
                # noinspection PyArgumentList
                # print s,t, nx.shortest_path_length(G, source=s, target=t)
                if sl[s][t] >= minShortestPath and degrees[s] >= minDegree:
                    result.append((s, t))
        return result
...
class FatTreeTopology(Topology):
    """ Represents a Fat tree topology """

    def __init__(self, name, graph=None, numFlows=0):
        """ Create a new FatTree topology

        :param name: the topology name
        :param graph: network graph containing the topology
        :param numFlows: number of flows for this topology
        """
        super(FatTreeTopology, self).__init__(name, graph, numFlows)

    def generateIEPairs(self, *args, **kwargs): # fuck it's a hack
        """ Compute IE pairs. Ingress and egress will be distinct for all
            pairs, and only between the edge layer switches

        :return: the list of IE pairs where ingress and egress are disc
        """
        G = self._graph
        leaves = [x for x in G.nodes_iter() if G.node[x]['layer'] == 'edge']
        product = itertools.product(leaves, repeat=2)
        # note we don't want commodities from src to src
        return [p for p in product if not p[0] == p[1]]


    def generateTrafficMatrix(self, nodePairs, model='uniform'):
        """ Generate the traffic matrix for this topology

        :param nodePairs: node pairs to use
        :param model: traffic model. Currently supports uniform only
...
class TestTopology(Topology):
    """ Topology used for pytest tests
    """

    def __init__(self, name, graph, numFlows=0):
        """ Creates a new topology

        :param name: name of the topology
        :param graph: :py:module:`networkx` graph
        :param numFlows: number of flows in this topology
        """
        super(TestTopology, self).__init__(name, graph, numFlows)

    def generateTrafficMatrix(self, nodePairs, model='uniform'):
        """ Generates the traffic matrix

        :param nodePairs: IE pairs
        :param model: traffic model. Uniform supported only
        :return: the traffic matrix
        :raises ValueError: if unknown traffic model is given
        """
        if model == 'uniform':
            return self.computeUniformODTM(nodePairs)
        else:
    ...
    def generateIEPairs(self):
        ...
        :return: all node combinations
        :rtype: list of tuples
        """
        return [(x, y) for x, y in itertools.product(self._graph.nodes(),
                                                     repeat=2)
                if x != y]
