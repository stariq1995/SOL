...
cpdef select_sa(apps, Topology topo, int num_paths=5, int max_iter=3,
    ...
    for k in arange(1, max_iter):
        ...
        for tcid in bs:
            ...
            for ii, pindex in enumerate(bestpaths[tcid].findall('0b1')):
                # The path should have been used in at least one epoch to be considered good
                vars = bestopt.get_xps()
                if all([x.x != 0 for x in vars[tcid,ii,:]]):
