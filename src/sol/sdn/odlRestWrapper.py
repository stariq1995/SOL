...
class ODLInterface(object):
    def __init__(self, controllerhost="localhost:8181", user='admin', password='admin', numworkers=20):
        """
        ...
        self._channel = goless.chan()
        # start up workers:
        for i in xrange(numworkers):
            goless.go(self.putWorker, self._channel)
                ...
                for ethSrc in srcMacList:
                    for ethDst in dstMacList:
                        ...
                        newFlow = self.buildFlow(flowName=flowName, tableId=0, flowId=flowId, inPort=inPort,
                                                 ...
                                                 installHw=installHw, priority=priority, nodeId=nodeId,
                                                 etherType='2048')

                        url = self.odlurl + '/config/opendaylight-inventory:nodes/node/' + nodeId + '/table/0/flow/' + str(
                            flowId)
                        flowId = flowId + 1
                        # print url
                        if self.parallel:
    ...
    def putPaths(self, paths):
        pass

    def putFlow(self, url, newFlow):
        requests.put(url, data=json.dumps(newFlow), headers={'content-type': 'application/json'}).raise_for_status()

    def putWorker(self, channel):
        pass
